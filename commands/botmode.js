...
const checkAdminOrOwner = require('../utils/checkAdmin'); // ton utilitaire centralisé
...
module.exports = {
  ...
  run: async (kaya, m, msg, store, args) => {
    ...
    if (!['on', 'off'].includes(value)) {
      return kaya.sendMessage(m.chat, {
        text: '❌ Valeur invalide. Utilise on ou off.',
        contextInfo
      }, { quoted: m });
    }
    ...
    if (cmd === 'public') {
      ...
      return kaya.sendMessage(m.chat, {
        text: `✅ Mode public du bot : ${value === 'on' ? 'activé' : 'désactivé'}`,
        contextInfo
      }, { quoted: m });
    }
    ...
    if (cmd === 'private') {
      ...
      return kaya.sendMessage(m.chat, {
        text: `✅ Mode privé du bot : ${value === 'on' ? 'activé' : 'désactivé'}`,
        contextInfo
      }, { quoted: m });
    }

    return kaya.sendMessage(m.chat, {
      text: '❌ Commande inconnue. Utilise .public ou .private.',
      contextInfo
    }, { quoted: m });
  }
};